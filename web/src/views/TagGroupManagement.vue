...
import { apiGetTagGroups, apiCreateAuthor, apiUpdaeteAuthor, apiRemoveAuthor, TagGroup } from '@/apis/tag-group'
import { ElForm } from 'element-plus'

const isOpen = ref(false)
const page = ref(1)
const data = reactive<{ list: TagGroup[]; total: number }>({ list: [], total: 0 })

const getList = async () => {
  const res = await apiGetTagGroups({ page: page.value })
  Object.assign(data, res)
}

const formRef = ref<InstanceType<typeof ElForm>>()
const rules = { name: [{ required: true, message: '请输入名称' }] }
const form = reactive<TagGroup>({ _id: '', name: '' })
const isCreate = computed(() => !form._id)

const onEdit = (item?: TagGroup) => {
...
<template>
  ...
  <el-drawer v-model="isOpen" title="标签分组编辑" size="60%">
